...
/**
 * Выполняет GET-запрос к APIfootball.com и возвращает массив результатов.
 * Если API вернул не-массив, возвращаем пустой массив.
 */
...
/** Парсит строку "YYYY-MM-DD HH:mm:ss" в JS Date (UTC) */
function parseMatchDate(str: string): Date {
  return new Date(str.replace(' ', 'T') + 'Z');
}

/** Форматирует дату и время на выходе для турецкой локали */
...
/** Преобразует "сырой" объект из API в ваш Match */
...
/**
 * Возвращает массив Match для:
 * - 'live'     → текущие live-матчи
 * - 'today'    → матчи на сегодня
 * - 'upcoming' → матчи следующих 7 дней (status=NS)
 */
export const fetchFootballScores = async (
  ...
  try {
    ...
    // Инициализируем params пустым, чтобы TS не жаловался
